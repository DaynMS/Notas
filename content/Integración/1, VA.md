...
<span class="defn">Definición:</span> Sea $f : [a,b] \to \R$ una función monótona, se define la **variación** de $f$ como $$V_f(a,b) = | f(b) - f(a)|$$
...
<span class="teor">Propiedad:</span> Si $P'$ es un refinamiento de $P$ (esto es $P \sube P'$) entonces $$V_f(a,b;P) \le V_f(a,b;P')$$

<span class="defn">Definición:</span> La **variación** de $f$ en el intervalo $[a,b]$ se define como $$V_f(a,b) = \sup_P V_f(a,b;P)$$
...
como no depende de $P$, es decir que se tiene para cualquier partición, corresponde a la variación. 
</div>
</details>
...
1. $f+\alpha g$ es VA y
$$
V_{f+\alpha g} \le V_f + |\alpha|V_g
$$

2. $f\cdot g$ es VA y existen $A,B \in \R$ tales que

$$
V_{f \cdot g} \le AV_f + BV_g
$$
...
<span class="defn">Definición:</span>  Sea $f: [a,b] \to \R^n$ continua, se define la curva asociada a $f$,

$$
C_f := \{ f(t) : t \in [a,b]\}
$$

$f$ se dice un **camino**, **trayectoria** o **parametrización** de $C_f$.
...
<span class="defn">Definición:</span> Un camino $f:[a,b] \to \R^n$ se dice regular si y sólo si
$$
f \in \mathcal C^1([a,b])
$$

para este caso se puede mostrar que
$$
L = \int_a^b \| f'(t) \| \ dt
$$
...
Sea  $f:[a,b] \to \R^n$ un camino regular y $c \in (a,b)$, entonces
$$
L_a^b = L_a^c + L_c^b
$$

## Función longitud de arco
<span class="defn">Definición:</span> Sea $f$ un camino rectificable en $[a,b]$, la longitud de arco está dada por:
